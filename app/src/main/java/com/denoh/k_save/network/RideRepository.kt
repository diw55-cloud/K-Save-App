...
class RideRepository {
    ...
    init {
        ...
        // Ensure ~70+ places are represented
        places.forEachIndexed { index, p ->
            if (p != "CBD") {
                // Rides TO the destination
                ...
                // Rides FROM the destination TO CBD
                allRides.add(RideOption(allRides.size, "Bolt Comfort 🚗", "$p → CBD", "Light", 20, 300 + (index % 80), "Ride", "https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=800&q=80"))
            }
        }

        // --- 2. Rentals (Premium Fleet) ---
        ...
        allRides.add(RideOption(3002, "Flight 540 ✈️", "JKIA → Kisumu", "Boarding", 0, 6200, "Airport", "https://images.unsplash.com/photo-1569154941061-e231b4725ef1?auto=format&fit=crop&w=800&q=80"))
    ...
    suspend fun getRidesByCategory(category: String, destination: String = ""): List<RideOption> {
        ...
        return allRides.filter { ride ->
            val isTargetCategory = ride.category.equals(category, ignoreCase = true) || 
                                   (category == "Ride" && (ride.category == "Ride"))
            
            val isTargetRoute = when {
                destination.isEmpty() -> true
                ride.category == "Rentals" || ride.category == "Package" -> true
    ...
    suspend fun getAllCategories(): List<String> = listOf("Ride", "Package", "Rentals", "Airport")
}
